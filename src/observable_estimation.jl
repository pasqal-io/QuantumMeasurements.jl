...
function single_estimate_obs(shadow::Shadow, obs::String)::Float64
    ...
    prod([
        real(
            tr(
                Matrix(mat(pauli_from_char[obs[j]]))' *
                chain_inverse(Matrix(mat(U[j])), b[j]),
            ),
        ) for j = 1:shadow.n_qubits
    ])
end
...
function robust_estimate_obs(
    ...
    R = Ne * Ke
    median_of_mean(
        [robust_single_estimate(calibration, shadows[r], observable) for r = 1:R],
        Ne,
        Ke,
    )
end
