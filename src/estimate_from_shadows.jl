...
function estimate_from_shadows(
    ...
    if Ne * Ke <= length(shadows)
        println(
            "Predictions have an accuracy of $precision with probability at least $probability",
        )
    else
        println(
...
function estimate_from_shadows(
    ...
    println(
        "Using $(Nc*Kc) classical shadows split into $Kc equally-sized parts for calibration",
    )
